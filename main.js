...
function ktskin() {
  var jjj = localStorage.getItem('dsskin')
  if (jjj === null) {
    var kkk = document.getElementById('mhtb')
    const span2 = document.getElementById('span2')
    kkk.style.display = 'block'
    
    var dsskin = ""
    var tatca = ids.length*25
    var gtso = 0
    for (var skin of ids) {
      var idh = parseInt(skin.substring(0,3) + "01")
      for (var i = idh; i < idh + 25; i++) {
        var test = new Image()
        test.src = 'https://dl.ops.kgtw.garenanow.com/CHT/HeroTrainingLoadingNew_B36/' + i + '.jpg'
        function ktskin(test, i) {
          test.onload = function () {
            dsskin += "_" + String(i)
            gtso++
            span2.textContent = String((gtso / tatca * 100).toFixed(2)) + "%"
            
            if (gtso === tatca) {
              localStorage.setItem('dsskin', dsskin)
              console.log(dsskin)
    ...
    kkk.textContent = "Vui lòng chờ..."
    
    var tatca = ids.length*25
    var gtso = 0
    var new_id = []
    
    dsskincu = localStorage.getItem('dsskin')
    
    for (var skin of ids) {
      var idh = parseInt(skin.substring(0,3) + "01")
      for (var i = idh; i < idh + 25; i++) {
        if (!dsskincu.includes(String(i))) {
          var test = new Image()
          test.src = 'https://dl.ops.kgtw.garenanow.com/CHT/HeroTrainingLoadingNew_B36/' + i + '.jpg'
          function ktskin(test, i) {
            test.onload = function () {
              if (!dsskincu.includes(String(i))) {
                new_id.push(String(i))
              }
              gtso++
              kkk.textContent = "Vui lòng chờ... " + String((gtso / tatca * 100).toFixed(2)) + "%"
              if (gtso === tatca) {
                xong(new_id, kkk)
              }
            }
...
function xong (new_id, kkk) {
  if (new_id.length == 0) {
    kkk.textContent = 'Không có skin mới! Chạm vào thông báo để tắt.'
    kkk.onclick = function () {
      kkk.style.display = 'none'
    }
  }
  else {
    var dsmoi = new_id.join(' ') + '. Chạm vào thông báo để tắt.'
    kkk.textContent = dsmoi
    kkk.onclick = function() {
        kkk.style.display = 'none'
    }
    localStorage.setItem('dsskin', dsskincu + "_" + new_id.join("_"))
  }
}